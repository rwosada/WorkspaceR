...
m <- c(10,40,60,20)
f <- c(21,60,70,30)
score <-  matrix(c(m,f), nrow = 4)  # 방법 1
score

score <- data.frame(m,f)            # 방법 2
score
...
#9. Florida 주의 모든 정보 출력                                 // Florida = 관측치, 데이터
st[ 'Florida', ]

#10. 50개 주의 수입(Income) 정보만 출력                         // Income = 변수
st[ , 'Income' ]

#11. Texas 주의 면적(Area)을 출력                               // Texas = 관측치, 데이터, Area = 변수
st[ 'Texas', 'Area' ]

#12. Ohio 주의 인구(Population)와 수입(Income) 출력             // Ohio = 관측치, c( 'Population', 'Income' ) = 변수
st[ 'Ohio', c( 'Population', 'Income' ) ]

#13. 인구가 5,000 이상인 주의 데이터만 출력        // subset(데이터이름 , 속성 == 속성값) : 부분적으로 가져오느것 
